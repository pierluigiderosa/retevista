...
var map = L.map('map').setView([43.08, 12.43], 8);

L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoicGllcmx1aWdpZGVyb3NhIiwiYSI6ImNpdzNuZnRjbjAwMHUyem1vMDhydno3MmsifQ.uwNNZ0OAQKWf4_7AbQUJoA', {
    maxZoom: 18,
...
var catasto = L.geoJson(catastoData,{
    style: stylelayer.defecto,
    onEachFeature: onEachFeature
}).addTo(map);
...
function checkExistsLayers(feature) {
    ...
    for (var i = 0; i < featuresSelected.length; i++) {
        if (featuresSelected[i].zipcode == feature.properties.zipcode) {  //TODO: qui cambia a particella
            result = true;
            break;
        }
...
info.update = function(properties) {
    this._div.innerHTML =
        ...
        '<h4>Properties</h4>' + (properties ?
            `
                Particella: ${properties.particella}<br>
                Foglio: ${properties.foglio}<br>
                Comune:${properties.comune}
                
                    ` : 'Passa su una particella');;
};
...
var detailshow = function() {
    ...
    for (var i = 0; i < featuresSelected.length; i++) {
        ...
        Foglio: ${properties.foglio}<br>
        Particella: ${properties.particella}<br>
        Comune:${properties.comune}<br>
        zipcode:${properties.zipcode}
        <a href="#" onclick=dellayer(${properties.particella})>Delete</a>
        <hr>`;
...
function dellayer(zipcode) {
    catasto.eachLayer(function(layer) {
